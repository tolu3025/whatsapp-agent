...
class FlutterwaveTokenManager {
    ...
    async getToken() {
        ...
        try {
            ...
            console.log('Flutterwave v4 token refreshed');
            return this.token;
        } catch (err) {
            console.error('Flutterwave token refresh failed:', err.response?.data || err.message);
            throw err;
        }
    }
}
...
const VendorSchema = new mongoose.Schema({
    ...
    onboardingStep: { 
        type: String, 
        enum: ["IDLE", "WAITING_BIZ_NAME", "WAITING_BANK", "WAITING_ACCT", "CONFIRMATION", "COMPLETED"],
        default: "IDLE" 
    },
    ...
    groupRules: { 
        type: String, 
        default: "Be polite, showcase our products, and tell them to DM us to order.",
        maxlength: 2000 
    },
...
app.get('/', function(req, res) { 
    res.status(200).json({ status: "KukaTai Agent Active", timestamp: new Date().toISOString() }); 
});
...
const COMMON_BANKS = {
    ...
    "fidelity": "070", "ecobank": "050", "union": "032", "sterling": "232"
};
...
async function checkRateLimit(vendorPhone) {
    ...
    if (now - resetTime > 3600000) {
        await Vendor.updateOne(
            ...
            { messageCount: 1, messageCountResetAt: now }
        );
        return { allowed: true, count: 1 };
    }
    
    if (vendor.messageCount >= 30) {
        return { allowed: false, retryAfter: Math.ceil((resetTime - now + 3600000) / 1000) };
    }
...
async function handleVendorSetupAndOnboarding(sock, msg, textMessage, lowerText) {
    ...
    if (vendor.onboardingStep === "COMPLETED" && !lowerText.startsWith('/')) {
        ...
        if (!rateLimit.allowed) {
            await sock.sendMessage(senderJid, { 
                text: 'Rate limit reached. Please wait ' + rateLimit.retryAfter + 's before sending more messages.' 
            });
            return true;
        }
    }
    ...
    if (lowerText.startsWith('/setrules ')) {
        ...
        if (rules.length > 2000) {
            await sock.sendMessage(senderJid, { text: 'Rules too long. Max 2000 characters.' });
            return true;
        }
    ...
    if (lowerText === '/linkgroup') {
        ...
        await vendor.save();
        await sock.sendMessage(senderJid, { 
            text: 'Add me to your WhatsApp Group, then type `/here` inside that group so I can capture its ID!' 
        });
        return true;
    }
    ...
    if (vendor.onboardingStep === "WAITING_BIZ_NAME") {
        ...
        if (bizName.length < 2 || bizName.length > 100) {
            await sock.sendMessage(senderJid, { text: 'Business name must be 2-100 characters. Try again:' });
            return true;
        }
        vendor.tempData = { businessName: bizName };
        vendor.onboardingStep = "WAITING_BANK";
        await vendor.save();
        await sock.sendMessage(senderJid, { 
            text: 'Nice! Now reply with your *Bank Name* (e.g. GTBank, Opay, Kuda):' 
        });
        return true;
    }
    ...
    if (vendor.onboardingStep === "WAITING_BANK") {
        ...
        if (!bankCode) {
            await sock.sendMessage(senderJid, { 
                text: 'Bank not recognized. Supported banks: Access, GTBank, Zenith, UBA, Opay, Kuda, Moniepoint, PalmPay, FirstBank, Wema, Fidelity, EcoBank, Union, Sterling. Try again:' 
            });
            return true;
        }
        vendor.tempData = Object.assign({}, vendor.tempData, { bankCode: bankCode, bankName: textMessage.trim() });
        vendor.onboardingStep = "WAITING_ACCT";
        await vendor.save();
        await sock.sendMessage(senderJid, { 
            text: 'Perfect! What is your *10-digit Account Number* for ' + vendor.tempData.bankName + ':' 
        });
        return true;
    }
    ...
    if (vendor.onboardingStep === "WAITING_ACCT") {
        ...
        try {
            const verifyRes = await flwRequest(
                'POST',
                '/banks/account-resolve',
                {
                    account: {
                        code: vendor.tempData.bankCode,
            ...
            if (verifyRes.data && verifyRes.data.status === 'success') {
                ...
                vendor.tempData = Object.assign({}, vendor.tempData, { accountNumber: accountNumber, accountName: accountName });
    ...
    if (vendor.onboardingStep === "CONFIRMATION") {
        if (lowerText === 'yes') {
            try {
                ...
                const recipientRes = await flwRequest('POST', '/transfers/recipients', {
                    type: "bank_ngn",
                    bank: {
                    ...
                    name: {
                        first: vendor.tempData.accountName.split(' ')[0] || 'User',
                        last: vendor.tempData.accountName.split(' ').slice(1).join(' ') || 'Name'
                    }
                });
                ...
                vendor.bankName = vendor.tempData.bankName;
                ...
                vendor.recipientId = recipientId;
                ...
                await sock.sendMessage(senderJid, { text: 'Setup error. Reply YES to try again or contact support.' });
            }
            ...
            await sock.sendMessage(senderJid, { text: 'Reply *YES* to confirm or *NO* to reset.' });
        }
        return true;
    }
...
async function handleImageUpload(sock, msg, vendor) {
    ...
    try {
        ...
        if (!buffer) {
            await sock.sendMessage(senderJid, { text: 'Could not download image. Try again.' });
            return;
        }
        ...
        vendor.savedPromoImages.push({
            url: urlData.publicUrl,
            caption: caption.substring(0, 500),
            uploadedAt: new Date()
        });
        
        if (vendor.savedPromoImages.length > 20) {
            vendor.savedPromoImages = vendor.savedPromoImages.slice(-20);
        }
        ...
        await sock.sendMessage(senderJid, { text: 'Failed to save image. Try again.' });
    }
}
...
async function startKukaTai() {
    ...
    for (let i = 0; i < 5; i++) {
        try {
            ...
            console.error('MongoDB attempt ' + (i+1) + '/5 failed: ' + err.message);
            await delay(5000);
        }
    }
    if (!mongoConnected) {
        console.error("MongoDB connection failed after 5 attempts. Exiting.");
        process.exit(1);
    }
    ...
    sock.ev.on('connection.update', async function(update) {
        ...
        if (qr) {
            console.log('QR Code received (scan if not using phone pairing)');
        }
        ...
        if (connection === 'close') {
            ...
            if (shouldReconnect) {
                ...
                console.log('Logged out. Clear auth folder and restart to pair again.');
            }
        } else if (connection === 'open') {
            console.log('KukaTai Agent Connected to WhatsApp!');
            if (!global.supabaseListenerStarted) {
                startSupabaseListener(sock);
                global.supabaseListenerStarted = true;
            }
            if (!global.transactionWatcherStarted) {
                startTransactionWatcher(sock);
                global.transactionWatcherStarted = true;
            }
        }
    });
        ...
        try {
            ...
            if (!isGroup) {
                ...
                if (vendor && vendor.onboardingStep === "COMPLETED") {
                    if (msg.message.imageMessage) {
                        await handleImageUpload(sock, msg, vendor);
                        return;
                    }
                    ...
                    try {
                        ...
                        console.error("OpenAI DM error:", err.message);
                        await sock.sendMessage(senderJid, { text: "I am having trouble thinking right now. Try again in a moment!" });
                    }
            ...
            if (isGroup) {
                ...
                if (lowerText === '/here') {
                    ...
                    if (!senderNum || senderNum === '@s.whatsapp.net') {
                        await sock.sendMessage(senderJid, { text: 'Could not identify sender.' });
                        return;
                    }
                    
                    const checkVendor = await Vendor.findOne({ phoneNumber: senderNum });
                    if (!checkVendor) {
                        await sock.sendMessage(senderJid, { text: 'You must register with KukaTai first. DM me *register* to get started.' });
                        return;
                    }
                    
                    if (checkVendor.onboardingStep !== "COMPLETED") {
                        await sock.sendMessage(senderJid, { text: 'Complete your registration first. Check your DMs for next steps.' });
                        return;
                    }
                    
                    const isAdmin = await isGroupAdmin(sock, senderJid, msg.key.participant);
                    if (!isAdmin) {
                        await sock.sendMessage(senderJid, { text: 'Only group admins can link this AI to the group.' });
                        return;
                    }
                    ...
                    await sock.sendMessage(senderJid, { 
                        text: '*AI Agent Activated!* I will now manage customer inquiries for *' + checkVendor.businessName + '* using your custom rules.' 
                    });
                    return;
                }
                ...
                if (vendor && vendor.onboardingStep === "COMPLETED") {
                    ...
                    const matchesKeyword = vendor.customKeywords.some(function(keyword) { 
                    ...
                    if (isMentioned || matchesKeyword) {
                        try {
                            ...
                            const aiResponse = await safeOpenAIChat(
                                systemPrompt, 
                                'From ' + (msg.pushName || "Customer") + ': ' + textMessage, 
                                150
                            );
                            
                            await sock.sendMessage(senderJid, { text: aiResponse }, { quoted: msg });
                        } catch (err) {
                            console.error("OpenAI Group error:", err.message);
                        }
                    }
                }
            }
        ...
        .on('postgres_changes', { 
            ...
            if (record.status === 'successful' || record.status === 'success') {
                ...
                try {
                    ...
                    console.error("Supabase Realtime Credit Error:", err.message);
                }
            }
        }).subscribe(function(status) {
            console.log('Supabase listener status: ' + status);
        });
}
...
async function startTransactionWatcher(sock) {
    try {
        ...
        console.log('MongoDB Change Streams not available (requires replica set):', err.message);
        console.log('Falling back to Supabase Realtime only');
    }
}
